...
针对BPH数据中gaoqing-T2模态数据不足（只有153例）的问题，项目提供了三种处理策略：

1. **零填充（zero_fill）**：对于缺失的模态，使用零值填充
2. **跳过（skip）**：只使用完整包含所有模态的病例
3. **复制（duplicate）**：使用其他模态的数据复制填充缺失模态

默认使用零填充策略，可以在训练配置中修改：
```python
config = {
    'handle_missing_modalities': 'zero_fill',  # 可选: 'zero_fill', 'skip', 'duplicate'
    # 其他配置...
}
```
